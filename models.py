...
class RNN_DecoderWithAttention(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, vocab_size, encoder_dim=2048, dropout=0.5):
        ...
        # Attention layer
        self.attention = Attention(encoder_dim, decoder_dim, attention_dim)  # 定义Attention层
        # Embedding layer
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=0)  # 嵌入层
        self.dropout = nn.Dropout(p=self.dropout)
        
        # RNNCell
        self.rnn = nn.RNNCell(embed_dim + encoder_dim, decoder_dim)  # 使用RNNCell
...
class RNNDecoderWithoutAttention:
